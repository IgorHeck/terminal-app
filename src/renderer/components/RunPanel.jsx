    ...
    <div className={`flex flex-col ${className}`}>
      <div className="h-9 flex items-center gap-2 px-3 flex-shrink-0">
        <span
          ...
          style={{
            background: dotColor(proc.status, project.color),
...
export default function RunPanel({
      ...
      <div className="flex-1 flex flex-row overflow-auto">
        {processes.map((p) => (
          <ProcessCard
            ...
            proc={p}
            fill
            className="flex-1 min-w-0 border-r border-border-soft"
            {...cardHandlers}
          />
        ))}
      </div>
          ...
          {processes.map((p) => (
            ...
            >
              <span
                ...
                style={{ background: dotColor(p.status, project.color) }}
              />
              {p.name}
            </button>
          ))}
  ...
  return (
    <div
      style={{ width }}
      className="flex-shrink-0 bg-panel border-l border-border-soft flex flex-col h-full"
    >
